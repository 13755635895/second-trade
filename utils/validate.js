 ...
 * @returns {boolean}
 */
export function isLicensePlateNumber(str) {
  const reg = /^(?:[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领 A-Z]{1}[A-HJ-NP-Z]{1}(?:(?:[0-9]{5}[DF])|(?:[DF](?:[A-HJ-NP-Z0-9])[0-9]{4})))|(?:[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领 A-Z]{1}[A-Z]{1}[A-HJ-NP-Z0-9]{4}[A-HJ-NP-Z0-9 挂学警港澳]{1})$/g;
  return reg.test(str);
}
/**
 ...
 * @returns {boolean}
 */
export function isMPStrict(str) {
 ...
 * @returns {boolean}
 */
export function isIDCardNew(str) {
 ...
 * @returns {boolean}
 */
export function isWebAccount(str) {
  const reg = /^[a-zA-Z]\w{4,15}$/g;
  return reg.test(str);
}
/**
